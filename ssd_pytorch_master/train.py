...
parser.add_argument('--resume', default='', type=str, #default='/home/emeka/Schreibtisch/AIS/ais3d/ssd.pytorch-master/weights/ssd300_KITTI_75000.pth'
                    help='Checkpoint state_dict file to resume training from')
    ...
    for epoch in range(start_epoch, epoch_size):
        for iteration, (images, targets) in enumerate(data_loader):
            ...
            if (iteration) != 0 and ((epoch)*iter_datasets +iteration) % 5000 == 0:
                ...
                torch.save(ssd_net.state_dict(), 'weights/ssd400_KITTI_' +
                           repr( (epoch)*iter_datasets +iteration ) + '.pth')
    #here the network is saved
    torch.save(ssd_net.state_dict(),
               args.save_folder + '' + args.dataset + '_' + repr(cfg['max_iter']) + '_400.pth')
    print('Network saved')
